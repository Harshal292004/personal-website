...
const TechSection = ({
  ...
  return (
    ...
    >
      ...
      >
        <h2
          className={`${jetbrains_mono.className} text-3xl sm:text-4xl md:text-5xl text-black dark:text-zinc-100`}
        >
          {title}
        </h2>
      </motion.div>
      ...
      <div className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 w-full md:w-3/4">
        {items.map((item, index) => (
          ...
          >
            ...
            <span
              className={`${fira_code.className} text-xs sm:text-sm md:text-base text-center dark:text-yellow-300`}
            >
              {item.text}
            </span>
          </motion.div>
