...
const TechStack = ({id}:{id:string}) => {
  ...
  return (
    <div id={id} className="w-full py-12 md:py-16 bg-green-50 dark:bg-zinc-900 bg-[linear-gradient(to_right,#80808033_1px,transparent_1px),linear-gradient(to_bottom,#80808033_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#ffffff12_1px,transparent_1px),linear-gradient(to_bottom,#ffffff12_1px,transparent_1px)] bg-[size:70px_70px] relative overflow-hidden transition-colors duration-300">
      {/* Decorative elements with responsive positioning */}
      <Star className="absolute top-8 right-8 md:top-12 md:right-12" color={theme === "light" ? "#43A047" : "#f1c40f"} size={30} />
      <Star className="absolute bottom-24 md:bottom-32 left-8 md:left-16" color={theme === "light" ? "#FF9800" : "#f39c12"} size={25} />
      <Star className="absolute top-1/3 left-12 md:left-24" color={theme === "light" ? "#2196F3" : "#e74c3c"} size={20} />

      {/* Mobile-only stars */}
      <Star className="absolute top-2/3 right-6 md:hidden" color={theme === "light" ? "#9C27B0" : "#f39c12"} size={15} />
      
      {/* Large screen only stars */}
      <Star className="hidden md:block absolute top-1/2 right-36" color={theme === "light" ? "#E91E63" : "#f1c40f"} size={40} />
      ...
      <div className="container mx-auto px-4 md:px-6">
        <TechSection title="Languages" items={LANGUAGES} isRightAligned={false} />
        <TechSection title="Frameworks" items={FRAMEWORKS} isRightAligned={true} />
        <TechSection title="Tools" items={TOOLS} isRightAligned={false} />
        <TechSection title="Databases" items={DATABASES} isRightAligned={true} />
      </div>
    </div>
  );
};
