    ...
    >
      ...
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 relative z-10 flex flex-col items-center">
        ...
        >
          ...
          <div
            className="absolute -top-1 -right-1 xs:-top-1.5 xs:-right-1.5 sm:-top-2 sm:-right-2 md:-top-3 md:-right-3 lg:-top-4 lg:-right-4 
                         w-3 h-3 xs:w-4 xs:h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-12 lg:h-12 
                         bg-green-400 dark:bg-gray-700 
                         border border-black dark:border-gray-200 z-10 rotate-12 transition-colors duration-300"
          ></div>
          <div
            className="absolute -bottom-1 -left-1 xs:-bottom-1.5 xs:-left-1.5 sm:-bottom-2 sm:-left-2 md:-bottom-3 md:-left-3 lg:-bottom-4 lg:-left-4 
                         w-3 h-3 xs:w-4 xs:h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-12 lg:h-12 
                         bg-emerald-400 dark:bg-gray-600 
                         border border-black dark:border-gray-200 z-10 rotate-45 transition-colors duration-300"
          ></div>
        </motion.div>

        {/* Loading text animation - Enhanced responsive font size */}
        <div className="flex justify-center mb-3 xs:mb-4 sm:mb-6 lg:mb-8">
          {["P", "L", "E", "A", "S", "E", " ", "W", "A", "I", "T", "..."].map(
            (letter, index) => (
              <motion.span
                key={index}
                custom={index}
        ...
        <div
          className="w-full max-w-xs xs:max-w-sm sm:max-w-sm md:max-w-md lg:max-w-lg 
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span
              className={`${jetbrains_mono.className} text-black dark:text-gray-200 font-bold text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl z-10`}
            >
              {Math.round(progress)}%
            </span>
          </div>
          ...
          <div
            className="absolute top-1/2 right-1 xs:right-1.5 sm:right-2 md:right-3 lg:right-4 
                         ...
                         transform -translate-y-1/2 rotate-45 transition-colors duration-300"
          ></div>
        </div>
